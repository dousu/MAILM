...
bool MAILMAgent::SYM_FLAG = false;
bool MAILMAgent::MUT_FLAG = false;
bool MAILMAgent::EXC_FLAG = false;
bool MAILMAgent::OMISSION_FLAG = false;
...
Rule
MAILMAgent::dither_say(std::vector<Rule>& internals) {
    try {
...
void
MAILMAgent::hear(std::vector<Rule>& terms, std::vector<Rule> all_meanings) {
...
std::vector<Rule>
MAILMAgent::think_meaning(std::vector<Rule>& internals) {
    //    std::cerr << internals.size() << std::endl;
    if (LOGGING_FLAG) {
...
void
MAILMAgent::sym_on(void) {
    SYM_FLAG = true;
    //  KnowledgeBase::logging_on();
}

void
MAILMAgent::mut_on(void) {
    MUT_FLAG = true;
    //  KnowledgeBase::logging_on();
}

void
MAILMAgent::exc_on(void) {
    EXC_FLAG = true;
    //  KnowledgeBase::logging_on();
}

void
MAILMAgent::omission_on(void) {
    OMISSION_FLAG = true;
    //  KnowledgeBase::logging_on();
}

void
MAILMAgent::omission_off(void) {
    OMISSION_FLAG = false;
    //  KnowledgeBase::logging_on();
}

std::vector<Rule>
MAILMAgent::random_think_meaning(std::vector<Rule>& internals) {
    //        std::cerr << "error" << std::endl;
...
void
MAILMAgent::dither_hear(std::vector<Rule>& terms, std::vector<std::vector<Rule> >& meaningss, std::vector<Rule>& all_meanings) {
...
MAILMAgent::think_meaning(bool& filled_flag, std::vector<Rule>& terms, std::vector<std::vector<Rule> >& meaningss, std::vector<Rule>& all_meanings) {
...
void
MAILMAgent::symmetry_bias_think(std::vector<Rule>& terms, std::vector<std::vector<Rule> >& meaningss, std::vector<Rule>& reference, std::vector<KnowledgeBase::PatternType>& patterns, std::vector<std::vector<Rule> >& return_rules) {
...
void
MAILMAgent::mutual_exclusivity_bias_think(std::vector<Rule>& terms, std::vector<std::vector<Rule> >& meaningss, std::vector<Rule>& reference, std::vector<KnowledgeBase::PatternType>& patterns, std::vector<std::vector<Rule> >& return_rules) {
    //候補がない場合、NULLを返す
    //exception未実装
    if (LOGGING_FLAG) {
...
void
MAILMAgent::mutual_exclusivity_bias_think(std::vector<Rule>& terms, std::vector<std::vector<Rule> >& meaningss, std::vector<Rule>& reference, std::vector<KnowledgeBase::PatternType>& patterns, std::vector<std::vector<Rule> >& term_pairs, std::vector<std::vector<std::vector<Rule> > >& meaning_pair_orders, std::vector<std::vector<double> >& meaning_distancess) {
    //候補がない場合、NULLを返す
    if (LOGGING_FLAG) {
        LogBox::push_log("USING MUTUAL EXCLUSIVITY BIAS THINK");
        LogBox::push_log("PROSPECTIVE MEANINGS:\n" + tr_vector_Rule_to_string(meaningss));
...
void MAILMAgent::decide_likelihood(std::vector<Rule>& terms, std::vector<std::vector<Rule> >& term_pairs, std::vector<std::vector<std::vector<Rule> > >& meaning_pair_orders) {
...
void MAILMAgent::symmetry_exception_check(Rule term, std::vector<Rule>& reference, std::vector<KnowledgeBase::PatternType>& patterns) {
...
void MAILMAgent::mutual_exclusivity_exception_check(Rule term, std::vector<Rule>& reference, std::vector<KnowledgeBase::PatternType>& patterns) {
    for (int i = 0; i < reference.size(); i++) {
...
Rule MAILMAgent::return_last_selected_meaning() {
