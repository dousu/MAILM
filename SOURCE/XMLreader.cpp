...
XMLreader::load(std::string file_path,
	...
	core_meaning[index_count].push_back(std::string("s") + std::to_string(file_no));
					...
					if (metric_t.first == "metric" &&
						...
						if (met_num == met_max) {
							...
							if (word_ex.size() != 0) {
								...
								core_meaning[index_count].push_back(std::string("c") + std::to_string((measure_t.second.get_optional<int>(
									(std::string("<xmlattr>.number")).c_str())
									).get()));
