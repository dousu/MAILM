...
template <typename T>
void Semantics<T>::store(AMean a, T v)
{
	if (mapping.count(a.get_obj_id()) == 0)
	{
		mapping[a.get_obj_id()] = v;
	}
	else
	{
...
template <typename T>
bool Semantics<T>::equal(AMean a, AMean b)
{
	...
	return mapping[a.get_obj_id()] == mapping[b.get_obj_id()];
}
//共通部分があればchunkできる
template <typename T>
bool Semantics<T>::chunk_equal(AMean a, AMean b)
{
	...
	//   return false;
	// }
	mapping[a.get_obj_id()].inter(mapping[b.get_obj_id()], tmp_v); //intersection
	return !tmp_v.empty();										   //共通部分あれば真
}
//aがbを含んでいればmergeできる
template <typename T>
bool Semantics<T>::merge_equal(AMean a, AMean b)
{
	T tmp_v;
	// if(mapping[a.obj].empty() || mapping[b.obj].empty()){
	//   // std::cout << mapping.size() << std::endl;
...
template <typename T>
bool Semantics<T>::replace_equal(AMean a, AMean b)
{
	T tmp_v;
	// if(mapping[a.obj].empty() || mapping[b.obj].empty()){
	//   // std::cout << mapping.size() << std::endl;
	...
	//   return false;
	// }
	return mapping[a.get_obj_id()].include(mapping[b.get_obj_id()]);
}
template <typename T>
void Semantics<T>::chunk(AMean a, AMean b, AMean c, AMean d, AMean e, int type, std::vector<Category> list)
{
	//type 1ならeにAMeanが入ってきているはず
	T c_v, d_v, e_v, null_v;
	mapping[a.get_obj_id()].diff(mapping[b.get_obj_id()], d_v, e_v);
	if (type == 1)
	{
		c_v = mapping[a.get_obj_id()] - d_v;

		if (c_v != mapping[b.get_obj_id()] - e_v)
		{
			std::cerr << "[chunk]MATCHING ERROR [chunk semantics]" << std::endl;
			exit(1);
		}
		mapping[c.get_obj_id()] = c_v;
		mapping[d.get_obj_id()] = d_v;
		mapping[e.get_obj_id()] = e_v;
		//Transrules
		...
		for (auto &rule : rules)
		{
			...
			ch_v.clear();
			outer_list.clear();
			while (next_pos < (int)rule.second.size())
			{				   //最後に調べた場所がendかどうかチェック
