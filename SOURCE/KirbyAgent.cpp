...
Rule KirbyAgent::say(Rule &internal)
{
	try
	{
...
void KirbyAgent::hear(Rule &term)
{
	kb.send_box(term);
}

void KirbyAgent::learn(void)
{
	kb.consolidate();
}

bool KirbyAgent::utterable(Rule &internal)
{
...
void KirbyAgent::logging_on(void)
{
	LOGGING_FLAG = true;
	KnowledgeBase::logging_on();
}

void KirbyAgent::logging_off(void)
{
	LOGGING_FLAG = false;
