...
template <typename T>
void Semantics<T>::chunk(const AMean &a, const AMean &b, const AMean &c, const AMean &c2, const AMean &d, const AMean &e, int pos,
  ...
  if (type == 1) {
    ...
    // b=>c
    std::for_each(std::begin(rules), std::end(rules), [&](auto &m) { rewrite(m.second, b, c); });
    // a=>cd
    std::for_each(std::begin(rules), std::end(rules), [&](auto &m) { rewrite(m.second, a, c, d, pos, d_size); });
  } else {
...
void Semantics<T>::merge(const AMean &a, const std::set<AMean> &b_vec, const AMean &c) {
  ...
  std::for_each(std::begin(b_vec), std::end(b_vec), [&](const AMean &b) {
    ...
    // a->c,b->c
    std::for_each(std::begin(rules), std::end(rules), [&](auto &m) { rewrite(m.second, a, c); });
    std::for_each(std::begin(rules), std::end(rules), [&](auto &m) { rewrite(m.second, b, c); });
...
void Semantics<T>::replace(const AMean &a, const AMean &b, const AMean &c, int b_pos, int b_size) {
  ...
  mapping[c] = c_v;
  // a=>cb
  std::for_each(std::begin(rules), std::end(rules), [&](auto &m) { rewrite(m.second, a, c, b, b_pos, b_size); });
...
void Semantics<T>::rewrite(Meaning &m, const AMean &a, const AMean &b) {
  ...
  std::for_each(std::begin(m.get_followings()), std::end(m.get_followings()),
                [&](MeaningElement &obj) { rewrite(obj.get<Meaning>(), a, b); });
}
template <typename T>
void Semantics<T>::rewrite(Meaning &m, const AMean &a, const AMean &b, const AMean &c, int pos, int size) {
  ...
  std::for_each(std::begin(m.get_followings()), std::end(m.get_followings()),
                [&](MeaningElement &obj) { rewrite(obj.get<Meaning>(), a, b, c, pos, size); });
}
template <typename T>
