...
bool Semantics<T>::merge_equal(const AMean &a, const AMean &b) const {
  ...
  return mapping.at(a).include(mapping.at(b)) ||
         mapping.at(b).include(
             mapping.at(a));  // a includes b or b includes a?
}
// aがbを含んでいればreplaceできる
template <typename T>
bool Semantics<T>::replace_equal(const AMean &a,
                                 const AMean &b) const {
  T tmp_v;
  // if (mapping.at(a).empty() && mapping.at(b).empty()) {
  //   return true;
...
template <typename T>
void Semantics<T>::chunk(const AMean &a, const AMean &b,
                         const AMean &c, const AMean &c2,
                         const AMean &d, const AMean &e, int pos,
                         int d_size, int e_size, int type) {
  ...
  if (type == 1) {
    ...
    if (c_v != mapping[b] - e_v) {
      std::cerr << "[chunk]MATCHING ERROR [chunk semantics]"
                << std::endl;
      exit(1);
    }
    mapping[c] = c_v;
    ...
    if (e_v == null_v) {
      c_v = mapping[a] - d_v;
      if (c_v != mapping[b] - e_v) {
        std::cerr << "[chunk2]MATCHING ERROR [chunk2 semantics]"
                  << std::endl;
        exit(1);
      }
      mapping[c] = c_v;
...
template <typename T>
void Semantics<T>::merge(const AMean &a, const std::set<AMean> &b_vec,
                         const AMean &c) {
  // cが同じだから違うbによってcが上書きされる
  mapping[c] = T();
  std::for_each(
      std::begin(b_vec), std::end(b_vec), [&](const AMean &b) {
        if (a == c || b == c) {
        ...
        // a->c,b->c
        std::for_each(std::begin(rules), std::end(rules),
                      [&](auto &m) { rewrite(m.second, a, c); });
        std::for_each(std::begin(rules), std::end(rules),
                      [&](auto &m) { rewrite(m.second, b, c); });
        if (a != b) mapping.erase(b);
      });
  mapping.erase(a);
}
template <typename T>
void Semantics<T>::replace(const AMean &a, const AMean &b,
                           const AMean &c, int b_pos, int b_size) {
  T c_v;
  c_v = mapping[a] - mapping[b];
  mapping[c] = c_v;
  // a=>cb
...
template <typename T>
void Semantics<T>::rewrite(Meaning &m, const AMean &a,
                           const AMean &b) {
  if (m.get_base() == a) {
    m = Meaning(b, m.get_followings());
  }
  std::for_each(std::begin(m.get_followings()),
                std::end(m.get_followings()),
                [&](MeaningElement &obj) {
...
void Semantics<T>::rewrite(Meaning &m, const AMean &a, const AMean &b,
                           const AMean &c, int pos, int size) {
  if (m.get_base() == a) {
    if (pos - 1 + size > m.get_followings().size()) {
      std::cerr << m << " target:" << a << " new1:" << b
                << " new2:" << c << " pos:" << pos << " size:" << size
                << std::endl
                << "error size : Semantics::rewrite" << std::endl;
      exit(1);
    }
    std::list<MeaningElement> buf, means;
    std::copy_n(std::begin(m.get_followings()), pos - 1,
                std::back_inserter(buf));
    std::copy_n(std::next(std::begin(m.get_followings()), pos - 1),
                size, std::back_inserter(means));
    buf.push_back(Meaning(c, means));
    std::copy(
        std::next(std::begin(m.get_followings()), pos - 1 + size),
        std::end(m.get_followings()), std::back_inserter(buf));
    m = Meaning(b, buf);
  }
  std::for_each(std::begin(m.get_followings()),
                std::end(m.get_followings()),
...
std::string Semantics<T>::rules_to_s(void) {
  ...
  std::for_each(std::begin(rules), std::end(rules), [&](auto &r) {
