...
int main(int arg, char **argv) {
  ...
  std::for_each(std::begin(v), std::end(v), [&](int n) {
    ...
    Reader::conv_alias.insert(
        std::map<std::string, std::string>::value_type(str2, str));
    Dictionary::symbol.insert(
        std::map<int, std::string>::value_type(n, str2));
    Dictionary::conv_symbol.insert(
        std::map<std::string, int>::value_type(str2, n));
  ...
  std::cout << "\n%%% previous" << std::endl;
  buf =
      Rule(LeftNonterminal(Category{-3}, Meaning(AMean{-7})), s_el4);
  kb.send_box(buf);
  ...
  std::cout << "\n%%% previous" << std::endl;
  buf =
      Rule(LeftNonterminal(Category{-4}, Meaning(AMean{-8})), s_el5);
  kb.send_box(buf);
  ...
  std::cout << r2 << std::endl
            << r3 << std::endl
            << std::boolalpha
            << (r2.get_external() == r3.get_external())
